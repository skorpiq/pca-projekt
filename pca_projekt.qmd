...
Správa je členená nasledovne: v @sec-teoria zhrnieme matematické pozadie metódy, @sec-data popisuje použitý dataset, v @sec-vypocet vykonáme samotný výpočet PCA a v @sec-vysledky prezentujeme výsledky s vizualizáciami. Závery sú zhrnuté v @sec-zaver.

# Teoretické pozadie {#sec-teoria}

## Kovariančná matica {#sec-kovariancia}
...
## Vlastné čísla a vlastné vektory {#sec-eigen}

Jadrom PCA je **spektrálny rozklad** kovariančnej matice definovanej v @eq-cov:
...
## Hlavné komponenty {#sec-komponenty}
...
## Algoritmus PCA — krok za krokom {#sec-algoritmus}

1.  Štandardizácia dát: $z_{ij} = \frac{x_{ij} - \bar{x}_j}{s_j}$
2.  Výpočet kovariančnej matice (@eq-cov): $\mathbf{C} = \frac{1}{n-1}\mathbf{Z}^\top\mathbf{Z}$
3.  Spektrálny rozklad (@eq-eigen): $\mathbf{C} = \mathbf{V} \boldsymbol{\Lambda} \mathbf{V}^\top$
...
Použijeme vstavaný R dataset `mtcars` [@r2024], ktorý obsahuje informácie o 32 modeloch automobilov z roku 1974. Prvých šesť riadkov datasetu zobrazuje @tbl-data.
...
Pred PCA je užitočné preskúmať korelácie — výsledná korelačná matica je znázornená na @fig-corr.
...
Z korelačnej matice na @fig-corr vidíme silné korelácie medzi premennými (napr. `disp`, `cyl`, `hp`, `wt`), čo naznačuje, že PCA dokáže tieto premenné efektívne zlúčiť.

# Výpočet PCA {#sec-vypocet}

## Ručný výpočet cez kovariančnú maticu

Najskôr ukážeme PCA "od základov" — priamo cez spektrálny rozklad podľa postupu zo @sec-algoritmus:
...
Záťaže prvých štyroch hlavných komponentov sú uvedené v @tbl-loadings.
...
loadings_df |>
  ...
  column_spec(2, bold = TRUE, color = ifelse(abs(loadings_df$PC1) > 0.3, "#8B0000", "#000000"))
```
...
Podiel variability vysvetlenej jednotlivými komponentmi (PVE, @eq-pve) znázorňuje scree plot na @fig-scree.
...
Ako vidno na @fig-scree, prvé dva komponenty vysvetľujú **`r round(cumulative_pve[2], 1)`%** celkovej variability, prvé tri komponenty **`r round(cumulative_pve[3], 1)`%**.

## Biplót — projekcia do 2D priestoru

Projekciu pozorovaní do roviny prvých dvoch komponentov spolu s vektormi záťaží zobrazuje @fig-biplot.
...
Záťaže premenných na prvých dvoch komponentoch (porovnaj s @tbl-loadings) sú vizualizované na @fig-loadings-plot.
...
**PC1** (vysvetľuje `r round(pve[1], 1)`% variability) je silne spojený s výkonom a veľkosťou motora (`hp`, `disp`, `cyl`, `wt`) — môžeme ho interpretovať ako **"silu vozidla"**. Premenné `mpg` a `drat` majú záporné záťaže (@fig-loadings-plot), čo znamená, že silnejšie autá majú vyššiu spotrebu.
...
1.  **Redukcia dimenzionality**: Z pôvodných 11 premenných stačia **2 komponenty** na vysvetlenie `r round(cumulative_pve[2], 1)`% variability, 3 komponenty pokrývajú `r round(cumulative_pve[3], 1)`% (pozri scree plot na @fig-scree).

2.  **Interpretácia**: PC1 reprezentuje "výkonnosť vozidla" (silne korelovaný s výkonom, objemom, hmotnosťou), PC2 reprezentuje "konfiguráciu motora" (@fig-loadings-plot).

3.  **Vizualizácia**: Biplót na @fig-biplot jasne ukazuje separáciu manuálnych a automatických prevodoviek v priestore PC1–PC2, čo naznačuje, že typ prevodovky súvisí s celkovou výkonnosťou vozidla.

4.  **Matematické základy**: PCA je elegantná aplikácia lineárnej algebry — spektrálny rozklad kovariančnej matice (@eq-eigen, @sec-teoria) poskytuje optimálnu lineárnu projekciu v zmysle maximalizácie rozptylu [@jolliffe2016].
